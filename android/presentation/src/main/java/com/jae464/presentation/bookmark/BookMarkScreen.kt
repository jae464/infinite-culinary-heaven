...
import com.jae464.presentation.contest.ContestHistoryIntent
...
fun BookMarkScreen(
    ...
    val isScrollingToEnd by remember(uiState.bookMarkedRecipes) {
        derivedStateOf {
            val lastVisibleItemIndex =
                listState.layoutInfo.visibleItemsInfo.lastOrNull()?.index ?: 0
            lastVisibleItemIndex >= uiState.bookMarkedRecipes.size - 2
        }
    }

    LaunchedEffect(isScrollingToEnd) {
        if (isScrollingToEnd && !uiState.isLoading && uiState.bookMarkedRecipes.size >= 20) {
    ...
    LaunchedEffectWithLifecycle {
        onIntent(BookMarkIntent.InitBookMarkedRecipes)
    }
